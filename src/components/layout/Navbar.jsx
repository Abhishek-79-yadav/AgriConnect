...
import { ArrowLeft, ShoppingCart, Heart, LogOut, LayoutDashboard, Search as SearchIcon, Sun, Moon } from "lucide-react";
...
import { consumeForwardNavigation, hasInAppHistory } from "../../hooks/useNavigationDepth";
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:gap-4 sm:px-6">
        {location.pathname !== "/" && (
          <button
            onClick={() => {
              // Same rule as the hardware back button: only step back if
              // we actually navigated here from another in-app screen —
              // otherwise (e.g. this was the very first screen opened,
              // like a deep link straight to Login) send them Home
              // instead of falling through to a blank/empty history state.
              if (hasInAppHistory()) {
            ...
            aria-label="Go back"
            className="-ml-1 shrink-0 rounded-full p-1 text-ink/70 transition hover:bg-card hover:text-ink md:hidden"
          >
            <ArrowLeft size={22} />
          </button>
        )}
        ...
        {/* Mobile: cart + theme toggle, pinned to the far-right corner now that
            there's no hamburger — the bottom nav's "More" sheet covers every
            other link this menu used to hold. */}
